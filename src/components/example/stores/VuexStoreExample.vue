...
const count = computed(() => store.getters["queue/getCount"]);

const queue = computed(() => store.getters["queue/getQueue"]);

const advanceQueue = () => store.commit("queue/moveLeft");

const leaveQueue = () => store.commit("queue/moveRight");

const addToQueue = () => {
  store.dispatch("queue/addItem", `QueueItem_${Date.now()}`);
};

const jumpQueue = () => {
  store.dispatch("queue/appendToQueue", `QueueItem_${Date.now()}`);
};

const removeFromQueue = (index: number) => {
  store.dispatch("queue/removeItem", index);
};
</script>
